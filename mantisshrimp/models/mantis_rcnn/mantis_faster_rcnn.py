...
class MantisFasterRCNN(MantisRCNN):
    ...
    def __init__(self, n_class, h=256, backbone="default", pretrained=True, metrics=None, **kwargs):
        super().__init__(metrics=metrics)
        self.n_class, self.h, self.backbone, self.pretrained = n_class, h, backbone, pretrained
        ...
        if(self.backbone == "defualt"):
            ...
            try:
                self.base_model = create_torchvision_backbone(backbone=self.backbone, is_pretrained=pretrained)
                self.ft_anchor_generator = AnchorGenerator(sizes=((32, 64, 128)), aspect_ratios=((0.5, 1.0, 2.0)))
                self.ft_mean = [0.485, 0.456, 0.406] # imagenet mean and std
                self.ft_std = [0.229, 0.224, 0.225]
                self.m = FasterRCNN(backbone=self.base_model, num_classes=self.n_class, image_mean=self.ft_mean, image_std=self.ft_std)
