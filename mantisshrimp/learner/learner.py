...
class Learner:
    ...
    def fit(
        self, max_epochs, lr, lr_sched_fn=None, gpus=None, callbacks=None, **kwargs
    ):
    ...
    @delegates(Trainer.__init__)
    def fit_one_cycle(self, max_epochs, lr_max, pct_start=0.25, **kwargs):
        def lr_sched_fn(opt):
            lrs = self.m.get_lrs(lr_max)
            sched = MultiCycleLR(
                opt, lrs, len(self.train_dl) * max_epochs, pct_start=pct_start
            )
            return {"scheduler": sched, "interval": "step"}

        return self.fit(
            max_epochs=max_epochs, lr=lr_max, lr_sched_fn=lr_sched_fn, **kwargs
        )
