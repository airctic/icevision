...
class AnnotationParser:
    ...
    def parse(self, show_pbar=True):
        ...
        for o in pbar(self, show_pbar):
            ...
            bbox = self.bbox(o)
            if notnone(bbox):
                bboxes[imageid].extend(L(bbox))
            mask = self.mask(o)
            if notnone(mask):
                masks[imageid].extend(L(mask))
            imageids.add(imageid)
        for d in [bboxes, masks, labels, iscrowds]:
            d.default_factory = lambda: None
        return [
            Annotation(
                i, labels[i], bboxes=bboxes[i], masks=masks[i], iscrowds=iscrowds[i]
            )
            for i in imageids
        ]
