...
class DataParser:
    ...
    def category_parser(self, data):
        raise NotImplementedError

    def info_parser(self, data, source):
        raise NotImplementedError

    def annotation_parser(self, data, source, catmap, idmap):
        raise NotImplementedError

    def parse(self, show_pbar=True):
        catmap = self.catmap or self.category_parser(self.data).parse_dicted(show_pbar)
        info_parser = self.info_parser(self.data, self.source)
        annotation_parser = self.annotation_parser(
            self.data, self.source, catmap=catmap, idmap=info_parser.idmap
        )
        imgs = L(info_parser.parse_dicted(show_pbar))
        annots = L(annotation_parser.parse_dicted(show_pbar))
        ...
        if not valid_iids.issubset(img_iids):
            raise ValueError(
                f"imageids {valid_iids - img_iids} present in annotations but not in images"
            )
        valid_imgs = imgs.filter(lambda o: o.imageid in valid_iids)
        print(
            f"Removed {len(imgs) - len(valid_iids)} images that don't have annotations"
        )
        # Sort and get items
