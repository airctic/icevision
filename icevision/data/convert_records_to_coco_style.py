...
def convert_record_to_coco_annotations(record):
    ...
    print("Create records to coco style")
    for bbox in record.detection.bboxes:
        print(f"Adding bbox.xywh {bbox.xywh}")
    ...
    print("convert_record_to_coco_annotations")
    if hasattr(record.detection, "masks"):
        # HACK: Hacky again!
        mask_array = record.detection.mask_array
        if mask_array is None:
            mask_array = MaskArray.from_masks(
                record.detection.masks, h=record.height, w=record.width
            )

        print(
            f"convert_record_to_coco_annotations::mask_array.shape {mask_array.shape}"
        )
