...
class Mosaic(BatchTransform):
    ...
    def apply(self, records: List[BaseRecord]) -> List[BaseRecord]:
        ...
        for i, current_record in enumerate(records):
            ...
            transformed_records.append(draw_mosaic([current_record, *mosaic_records]))

        # for record, tfm_record in zip(records, transformed_records):
        #     record = tfm_record
        return transformed_records


# TODO: get rid of RecordType?
def draw_mosaic(records: List[RecordType]) -> PIL.Image.Image:
    main_record = first(records)
    h = main_record.img_size.height
    w = main_record.img_size.width
    rw, rh = (1 + np.random.random_sample(2)) / 3
    pw, ph = int(rw * w), int(rh * h)

    crop_boundaries = [[0, 0, pw, ph], [pw, 0, w, ph], [0, ph, pw, h], [pw, ph, w, h]]
    ...
    mosaic_tfms = [
        tfms.A.Adapter(
            [
                tfms.A.Crop(*crop_boundaries[i]),
                tfms.A.PadIfNeeded(h, w, position=positions[i], border_mode=0),
            ]
        )
        for i in range(4)
    ]
    ...
    for record, tfm in zip(records, mosaic_tfms):
