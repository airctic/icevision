...
def get_model_types_for_backend_lib(
    lib_name: str, base_path: str = BASE_PATH
) -> List[str]:
    ...
    model_types = [
        i
        for i in os.listdir(os.path.join(base_path, lib_name))
        if os.path.isdir(os.path.join(base_path, lib_name, i))
        and not i.startswith("__")
    ]
    return model_types
...
def get_backbone_names(
    lib_name: str, model_name: str, base_path: str = BASE_PATH
) -> List[str]:
    ...
    backbone_files = [
        i
        for i in os.listdir(os.path.join(base_path, lib_name, model_name, "backbones"))
        if os.path.join(base_path, lib_name, model_name, "backbones", i).endswith(".py")
        and not i.startswith("__")
    ]
    backbone_list = []
...
def load_model_components(
    ...
    try:
        backend_lib = get_module_element_form_module(models, lib_name)
    except AttributeError:
        raise AttributeError(
            f"Backend lib: {lib_name} not found. Possible options are: {get_backend_libs(model_base_path)}"
        )
    # load the model_type
    try:
        model_type = get_module_element_form_module(models, lib_name, model_name)
    except AttributeError:
        raise AttributeError(
            f"Model type: {model_name} not found. Possible options are: {get_model_types_for_backend_lib(model_base_path, lib_name)}"
        )
    # load backbone
    try:
        backbone_module = get_module_element_form_module(
            models, lib_name, model_name, "backbones"
        )
        backbone = getattr(backbone_module, backbone_name)
    except AttributeError:
        raise AttributeError(
            f"Backbone: {backbone_name} not found. Possible options are: {get_backbone_names(lib_name, model_name)}"
        )
...
def build_model(
    ...
    if model_config is None:
        model_config = {}
    model_type, backbone = load_model_components(lib_name, model_name, backbone_name)
