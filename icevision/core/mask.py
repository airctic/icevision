...
class VocMaskFile(MaskFile):
    ...
    def to_mask(self, h, w) -> MaskArray:
        mask_arr = numpy_to_mask(
            image_to_numpy(
                open_img(self.filepath, gray=True, ensure_no_data_convert=True)
            )
        )
...
class SemanticMaskFile(Mask):
    ...
    def to_mask(self, h, w, pad_dim=True):
        ...
        mask = open_img(self.filepath, gray=True, ensure_no_data_convert=True)
        ...
        mask = numpy_to_mask(image_to_numpy(mask))
