...
#     coco_erles = rle.to_erles(h=17, w=1)
#     coco_rle = coco_erles.to_mask(h=17, w=1).to_coco_rle(h=17, w=1)
...
#     h = 1
#     w = 7
#     mask = MaskArray(np.array([[[0, 0, 1, 1, 1, 0, 1]]]).reshape(w, h, -1))
#     print(mask.shape)

#     assert mask.to_coco_rle(h=h, w=w) == [{"counts": [2, 3, 1, 1], "size": (h, w)}]

#     mask = MaskArray(np.array([[[1, 1, 1, 1, 1, 1, 0]]]))
#     print(mask.shape)
#     assert mask.to_coco_rle(h=h, w=w) == [{"counts": [0, 6, 1], "size": (h, w)}]
...
def test_voc_mask_file(samples_source):
    ...
    assert mask_arr.data.shape == (500, 375, 2)
...
#     h, w = 427, 640
#     mask = poly.to_erles(h=h, w=w).to_mask(h=h, w=w)

#     assert mask.shape == (w, h, 1)
