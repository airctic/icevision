...
def test_keypoints_parser(coco_dir, coco_keypoints_parser):
    records = coco_keypoints_parser.parse(data_splitter=SingleSplitSplitter())
    assert len(records[0]) == 2
    assert records[0][1].filepath == coco_dir / "images/000000404249.jpg"
    assert len(records[0][1].keypoints) == 1
    assert records[0][1].keypoints[0].n_visible_keypoints == 16
    assert records[0][1].keypoints[0].y.max() == 485
    assert len(records[0][0].keypoints) == 9
