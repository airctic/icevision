...
def filter_pull_requests(pull_requests, start_pull_request_id=None, end_pull_request_id=1, end_is_off_by_one=False):
...
def extract_contribution_data_from_pull_requests(pull_requests):
    ...
    for pull_request in pull_requests:
        if not pull_request.user.login in data.keys():
            data[pull_request.user.login] = [{"title": pull_request.title, "id": pull_request.number, "url": pull_request.comments_url}]
        else:
            data[pull_request.user.login].append({"title": pull_request.title, "id": pull_request.number, "url": pull_request.comments_url})
    return data
...
## {version_name}\nThe following PRs have been merged since the last version.\n\n"""
    ...
    markdown += "**Thank you to all contributers: " + ", ".join([entry[0] for entry in sorted_data]) + "**" + "\n\n[comment]: # (Version_end)\n\n"
    return markdown
...
def get_latest_pull_request_id(log_data):
    ...
    for contributer_pull_requests in log_data["latest_release"].values():
        pull_request_ids += [pull_request["id"] for pull_request in contributer_pull_requests]
...
def load_changelogs():
    ...
    with open("CHANGELOG_backup.md") as f:
        f.write(change_log)
    changelogs_head, changelog_body = change_log.split("[comment]: # (Add changes below)")
...
parser = argparse.ArgumentParser(description="""Automatically update the CHANGELOG.md file. The script can create an update between two given PRs, were all RPs including the start and end PR are listed for the new version. Or only one can be provided. 
...
parser.add_argument('github_token', metavar='T', type=str, help="Access token for github.")
parser.add_argument('version', metavar='V', type=str, help='String for the name of the next version')
parser.add_argument('--start_pr', default=None, type=int, help='Id of the pr to start the collection of PRs looking backward in time. This should be a bigger than the value of --end_pr.')
parser.add_argument('--end_pr', default=None, type=int, help='Id of the pr to end the collection of PRslooking backward in time. This sould be smaller than the value of --start_pr')
...
if __name__ == "__main__":
    ...
    selected_pull_requests = filter_pull_requests(pull_requests, end_pull_request_id=start_pull_request_id, start_pull_request_id=args.end_pr, end_is_off_by_one=end_pull_request_id_is_off_by_one)
    pull_requests_data = extract_contribution_data_from_pull_requests(selected_pull_requests)
    markdown_update = create_markdown_from_pull_request_data(pull_requests_data, args.version)

    # apply updates
    print("Updating logs and markdown.")
    log_update(changelog_data, args.version, pull_requests_data)
    update_changelog_file(create_markdown_from_pull_request_data(pull_requests_data, args.version))
